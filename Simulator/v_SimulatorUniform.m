...
% 2023-10-22: Add modules for simulating baseline.
% 2023-10-29: Modify the way to generate system noise. [Step 6]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [simuData] = v_SimulatorUniform(Chroma, Spec, ...
    idx_comp, uniTable, noiselevel, baseline, kwargs)
% ---------------------------------------------------------
%                    Initialize Factors 
% ---------------------------------------------------------
%
...
%   @.uniOrder  : The order to arrange the columns in Uniform design Table 
%                 to the selected componnets. Enter a vector.
%      (default): [1 : sz_comp].
%    value '-1' : To use random uniform distribution INSTEAD OF uniform
%                 design table.
%   @.concLOD   : The Limit of Detection (LOD) for each component.
%                 Enter a vector with size [sz_comp, 1].
%      (default): 10.00 (dimensionless)
%   @.concScaler: Multiples used to construct the concentration profiles.
%                 Enter a vector with size of [sz_comp, 1].
%      (default): 100 = maximum level / minimum level.
...
% where the concentration is designed based on the Uniform Design Table. 
...
if isfield(Chroma, 'paramEMG')
    ...
    for cp_ = 1 : DimX(4)
        ...
        if flag, CHROMA.data(axis(1) : axis(2), cp_) = Chroma.data{c_};
        else, CHROMA.data(axis(1) : axis(2), cp_) = Chroma.data(c_, :); end
    end
...
clear reg axis cp_ c_ rovEMG flag
...
% [Part 1.3] Load concentration profiles from Uniform Design Table
load(vpath + "\data\Conc_uniform.mat");
if isstring(uniTable)
    fidx = [str2double(regexp(uniTable, '\d*\.?\d*','match')), 2];
else
    switch length(uniTable)
        case 2, fidx = [uniTable([1,1,2], 2)];
        case 3, fidx = [uniTable, 2];
        case 4, fidx = uniTable;
    end
end
i_ = find(ConcUniform.fidx(:, 2) == fidx(2) & ConcUniform.fidx(:, 3) == fidx(3));
if isempty(i_), error("Uniform table not found!!!"); end
if fidx(4) > 1, fidx(4) = max(ConcUniform.fidx(i_, 4)); end
i_ = i_( ConcUniform.fidx(i_, 4) == fidx(4) );
if isempty(i_), error("Uniform table not found!!!"); end
DimX(3) = fidx(2);
CONC = struct();
CONC.data = zeros(DimX(3), DimX(4));
CONC.name = ConcUniform.name(i_);
CONC.info = ConcUniform.fidx(i_, :);
CONC.table = ConcUniform.table{i_, 1};
CONC.usage = ConcUniform.table{i_, 2};
sz_usage = sum(unique(kwargs.uniOrder) > 0);
CONC.D = CONC.usage(1, sz_usage);
for c_ = 1 : DimX(4)
    if kwargs.uniOrder(c_) < 1
        if kwargs.concScaler(c_) >= 1
            CONC.data(:, c_) = randi([1, DimX(3)], DimX(3), 1);
        end
    else
        uo = kwargs.uniOrder(c_);
        CONC.data(:, c_) = CONC.table(:, CONC.usage(sz_usage, uo));
    end
end
clear fidx i_ ConcUniform c_ uo sz_usage

% [Part 1.4] Check and complete the 'kwargs' Struct
if ~isfield(kwargs, 'shift'), kwargs.shift = []; end
if ~isfield(kwargs, 'uniOrder'), kwargs.uniOrder = 1 : DimX(4); end
if ~isfield(kwargs, 'concLOD'), kwargs.concLOD = 10 * ones(1 : DimX(4)); end
if ~isfield(kwargs, 'concScaler'), kwargs.concScaler = 100 * ones(1, DimX(4)); end
...
% ****************************************
%   [Step 3] Randomly simulate concentration according to Uniform Design
% ****************************************
concu = double((1 : CONC.info(2))');
for c_ = 1 : DimX(4)
    if kwargs.concScaler(c_) > 1
        conc_level = concu(1) + (concu - 1) * ...
            ( (kwargs.concScaler(c_)-1) / double(CONC.info(2)-concu(1)) ); 
        CONC.data(:, c_) = conc_level(CONC.data(:, c_));
    end
end
if iscolumn(kwargs.concLOD), kwargs.concLOD = kwargs.concLOD'; end
conc_base = abs(randn(1, DimX(4))) + kwargs.concLOD;
CONC.data = CONC.data .* conc_base;
clear concu c_ conc_level conc_base
...
simuData.param.uniTable = CONC.name;
...
simuData.param.uniOrder = kwargs.uniOrder;
simuData.param.concLOD = kwargs.concLOD;
simuData.param.concScaler = kwargs.concScaler;
